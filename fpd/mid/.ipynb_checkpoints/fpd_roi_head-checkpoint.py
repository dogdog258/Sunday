...
class FPDRoIHead(StandardRoIHead):
    ...
    def _optimized_bbox_forward_train(self, query_feats: List[Tensor],
        ...
        supp_ids = []
        num_supp_per_im = num_supp_shots + 2
        num_img = 3
        for img_id in range(batch_size):
            ...
            for i in range(support_feats[0].size(0)):
                if support_gt_labels[i] == random_query_label:
                    # supp_id.append(i)
                    mid.append(i)
            supp_id.append(np.random.choice(mid))
            while len(supp_id) < num_img:
                select_id = np.random.choice(range(support_feats[0].size(0)))
                # if supp_id[0] != select_id and supp_id[1] != select_id:
                if supp_id[0] != select_id:
                    if (select_id % 2) == 0:
                        supp_id.append(select_id)
                        supp_id.append(select_id + 1)
                    else:
                        supp_id.append(select_id - 1)
                        supp_id.append(select_id)
        ...
        for i in range(2 * batch_size):
            if i < batch_size:
                new_support_feat.append(support_feat[supp_order][i])
            else:
                n_sam_feat1 = lam * support_feat[supp_order][i]
                n_sam_feat2 = (1 - lam) * support_feat[supp_order][i+batch_size]
                n_sam_feat = n_sam_feat1 + n_sam_feat2
                new_support_feat.append(n_sam_feat)
        rpt_prototype = new_support_feat
            ...
            for key in single_loss_bbox.keys():
                ...
                if key == 'acc':
                    ...
                    if k == 0:
                        ...
                        loss_bbox[key].append((1 / (num_supp_per_im - num_supp_shots + 1)) * single_loss_bbox[key])
